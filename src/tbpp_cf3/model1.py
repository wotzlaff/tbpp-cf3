...
from .util import pairwise, compute_conflict_cliques, solve_bpps
...
def set_start(model: gp.Model, inst: InstanceTBPPFU, alloc: Collection[frozenset[int]]):
    ...
    for t, k in y:
        ys[t, k] = y[t, k].Start = (
            ...
            if (
                k < len(alloc)
                and any(inst.s[i] <= t and inst.e[i] > t for i in alloc[k])
            )
            else 0
        )
...
def _extract_solution(model: gp.Model, inst: InstanceTBPPFU):
    x = inst._vars['x']
    sol = []
    for k in range(inst.n):
        if (k, k) not in x:
            break
        pat = {i for i in range(k, inst.n) if x[i, k] > 0.5}
        if not pat:
            continue
        sol.append(pat)
    return sol
...
def build(
    ...
    assert mods <= {'conflicts', 'bpp', 'wy', 'continuous_w', 'all_ts'}
    all_ts = 'all_ts' in mods
    ...
    tsnd_k = {
        k: {t0 for t0, t1 in pairwise(t_k[k]) if t0 in ts_k[k] and t1 in te_k[k]}
        for k in idx_k
    }

    last_ts_k = {k: max(ts_k[k]) for k in idx_k}
    te_k = {
        k: {inst.e[i] for i in idx_i if i >= k and inst.e[i] < last_ts_k[k]}
        for k in idx_k
    }
    t_k = {k: sorted(ts_k[k] | te_k[k]) for k in idx_k}
    ...
    # add variables
    x = m.addVars(
        [(i, k) for i in idx_i for k in idx_k if i >= k], vtype=gp.GRB.BINARY, name='x'
    )
    ...
    w = m.addVars(
        ((t, k) for k in idx_k for t in (t_k if all_ts else ts_k)[k]),
        obj=inst.gamma,
        lb=0.0,
        ub=1.0,
        vtype=w_type,
    ...
    m._servers = z.sum()
    m._fireups = w.sum()
    m._extract_solution = lambda: _extract_solution(m, inst)
    ...
    m.addConstrs(
        (
            gp.quicksum(
                inst.c[i] * x[i, k]
                for i in idx_i
                if i >= k and inst.s[i] <= t and t < inst.e[i]
            )
            <= inst.cap * y[t, k]
            for k in idx_k
            for t in tsnd_k[k]
    ...
    m.addConstrs(
        (
            gp.quicksum(
                x[i, k] for i in idx_i if i >= k and inst.s[i] <= t and t < inst.e[i]
            )
            >= y[t, k]
            for k in idx_k
            for t in te_k[k]
    ...
    m.addConstrs((x.sum(i, '*') == 1 for i in idx_i), name='assign')

    # coupling of x and y
    m.addConstrs(
        (x[i, k] <= y[inst.s[i], k] for k in idx_k for i in idx_i if k <= i), name='act'
    )

    # coupling of y and z
    m.addConstrs(
        (y[t, k] <= z[k] for k in idx_k for t in tsnd_k[0] & ts_k[k]), name='use_y'
    )
    ...
    m.addConstrs(
        (
            ...
            for k in idx_k
            for tp, t in pairwise(chain(['s'], t_k[k]))
            if t in (t_k if all_ts else ts_k)[k]
        ),
        name='fireup',
    )
    ...
    if lb_servers > 0:
        m.addConstrs((z[k] == 1 for k in range(lb_servers)), name='lb_server')

    # break symmetry
    m.addConstrs(
        (z[k] >= z[k + 1] for k in idx_k[lb_servers:-1]), name='break_symmetry'
    )

    # add VIs
    m.addConstrs((z[k] <= w.sum('*', k) for k in idx_k), name='server_fireup')
    ...
    if 'bpp' in mods:
        bounds = solve_bpps(inst)
        m.addConstrs(
            (
                gp.quicksum(y[t, k] for k in idx_k if t in ts_k[k]) >= bounds[t]
